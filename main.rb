...
#6 -- Start game
	...
	#6 -- Preflop (Send out hole cards) (1st betting round)
		...
		printLine
		@table.each {|player| puts "#{player.name} #{player.turn} #{player.style} recieved: #{player.cards.join(' ')} | odds: #{player.odds}"}
...
#7 -- Create end game stats to database and display
	printLine
	puts "		STATISTICS"
	printLine
...
def startBetting
	...
	@table.each_with_index do |player, index|
		#2 -- Check table to see if everyone folded
		if @table.length == 1
			printLine
		...
		#2 -- Compute Computer decisions
		if player != @user
			#Fill style requirements to decide computer decisions
		...
		#3 -- Compute User decision if user hasn't folded
	...
	#Remove players who have folded from current players
	@table.removeFolded
...
#Cycle through players after a bet was made
def makeBets(index)
	puts "In makeBets"
	i = index
	@table.each do 
		if @table[i] != @user
			...
			@table[i].style.odds = @table[i].odds
			@table[i].style.starting_chip = @starting_chip
			@table[i].style.bankroll = @table[i].bankroll
			@table[i].style.pot_size = @table.pot_size
			@table[i].style.bet_made = true if @bet_made
			@table[i].style.bet_made = false if !@bet_made
			if @decision == 'bet'
				@bet_made = true
				puts "#{@table[i].name} bet $2!"
				@table.bet(2,@table[i])
			else
				@decision = @table[i].style.decide
				print @table[i].style.bet_made
				if @decision == 'call'
					puts "#{@table[i].name} called!"
					@table.bet(2,@table[i])
				else
					puts "#{@table[i].name} folded!"
					@table.fold(@table[i])
					@table[i].folds += 1
				end
			end	
...
#Evaluates and prints user's pokerhand odds
def getUserOdds
	evaluateOdds
...
#Evaluates all player's pokerhand odds
def evaluateOdds 
